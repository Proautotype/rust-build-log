...
export const getHomeXTrends = createServerFn({ method: "GET" }).handler(async () => {
  try {
    ...
    if (cache && Date.now() - cache.at < CACHE_MS && Array.isArray(cache.trends)) {
    ...
    console.error("getHomeXTrends failed", error);
    return { connected: false, trends: [] as HomeTrend[], stories: [] as HomeTrendStory[] };
  }
});
